...
<div class="container">
  ...
  <br><br>
  <h1 class="display-2 text-center">Müşteri Silme Sayfası</h1><br><hr><br>
  ...
  <form action="" method="post">
    Müşteri No:<br />
    <input type="number" name="musteriNo" required="required" /><br /><br>
...
if (isset($_POST['musteriNo'])) {
  ...
  if (empty($musteriNo)) {
    ...
    try {
      ...
      $sorgu = $pdo->prepare('DELETE FROM "Musteri" WHERE "musteriNo"=? ');
            $sorgu->bindParam(1, $musteriNo, PDO::PARAM_STR);
            $sorgu->execute();

            echo "<br><p>Müşteri Başarılı bir şekilde silindi</p>";
