...
<div class="container">
  ...
  <br><br>
  <h1 class="display-2 text-center">Müşteri Ekleme Sayfası</h1><br><hr><br>
...
if (isset($_POST['musteriNo'], $_POST['ad'], $_POST['soyad'], $_POST['cinsiyetNo'], $_POST['ilNo'])) {
  ...
  if (empty($musteriNo) || empty($ad) || empty($soyad) || empty($cinsiyetNo) || empty($ilNo)) {
    ...
    try {
      ...
      $sorgu = $pdo->prepare('INSERT INTO "Musteri"("musteriNo", "ad", "soyad","cinsiyetNo","ilNo") VALUES(?,?, ?,?,?)');
            $sorgu->bindParam(1, $musteriNo, PDO::PARAM_STR);
            $sorgu->bindParam(2, $ad, PDO::PARAM_STR);
            $sorgu->bindParam(3, $soyad, PDO::PARAM_STR);
            $sorgu->bindParam(4, $cinsiyetNo, PDO::PARAM_STR);
            $sorgu->bindParam(5, $ilNo, PDO::PARAM_STR);

            $sorgu->execute();

            echo "<p>Müşteri Başarılı bir şekilde eklendi</p>";
